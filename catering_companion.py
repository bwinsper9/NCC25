...
if num_guests and selected_recipes:
    ...
    shopping_pdf = generate_shopping_list_pdf(combined)
    recipe_pdf = generate_recipe_guides(filtered_df)

    with st.sidebar:
    st.markdown("### 📥 Download PDFs")
    with open(shopping_pdf.name, "rb") as f1, open(recipe_pdf.name, "rb") as f2:
        st.download_button("Download Shopping List PDF", data=f1, file_name="shopping_list.pdf", mime="application/pdf")
        st.download_button("Download Recipe Guides PDF", data=f2, file_name="recipe_guides.pdf", mime="application/pdf")
    ...
    with open(shopping_pdf.name, "rb") as file:
        st.download_button("Download Shopping List PDF", data=file, file_name="shopping_list.pdf", mime="application/pdf")
    ...
    recipe_pdf = generate_recipe_guides(filtered_df)
    with open(recipe_pdf.name, "rb") as file:
        st.download_button("Download Recipe Guides PDF", data=file, file_name="recipe_guides.pdf", mime="application/pdf")
